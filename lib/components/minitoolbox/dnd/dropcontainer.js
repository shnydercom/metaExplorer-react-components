...
exports.DropContainer = (props) => {
    ...
    const [isClickThrough, setIsClickThrough] = React.useState(!!props.isDropZoneClickthrough);
    const [, drop] = react_dnd_1.useDrop({
        accept: [ItemTypes_1.default.MiniToolBox, ItemTypes_1.default.Block],
        drop(item, monitor) {
            const delta = monitor.getDifferenceFromInitialOffset();
            const left = Math.round(item.left + delta.x);
            const top = Math.round(item.top + delta.y);
            if (item.type === ItemTypes_1.default.MiniToolBox)
